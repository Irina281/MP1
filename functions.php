...
{
	if( is_multisite() && is_author() )
	{
		$is_404 = true;
	}	
	else if( $wp_query->posts && is_singular('advert') && ( adifier_get_advert_meta( $wp_query->post->ID, 'sold', true ) == 1 || adifier_is_expired( $wp_query->post->ID ) ) )
	{
		$wp_query->set_404();
		status_header( 404 );
		nocache_headers();
