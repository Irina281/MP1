...
class Adifier_Admin_Custom_Fields{
	...
	*/ 
	public static function output() {
		...
		elseif ( !empty( $_REQUEST['delete_cf'] ) ) {
		...
		elseif ( !empty( $_REQUEST['delete_group'] ) ){
				...
				<div id="col-right">
					<div class="col-wrap">
						<form action="<?php echo esc_url( add_query_arg( array() ) ); ?>" method="post">
											...
											foreach ( $groups as $group ) :
												...
												?><tr>
													<th scope="row" class="check-column">
														<input type="checkbox" name="delete_group[]" value="<?php echo esc_attr( $group->group_id ) ?>" id="cb-select-<?php echo esc_attr( $group->group_id ) ?>">
													</th>
													<td>
														<strong>
							...
							<div class="cf-flex-btns">
								<button type="submit" class="button button-primary cf-bulk-delete"><?php esc_html_e( 'Delete Selected', 'adifier' ) ?></button>
							</div>
						</form>
					</div>
				</div>
	...
	*/ 
	private static function process_delete_group(){
		...
		$group_ids = !is_array( $_REQUEST['delete_group'] ) ? (array)absint( $_GET['delete_group'] ) : $_REQUEST['delete_group'];
									...
									<?php
										if ( $fields = self::get_custom_fields_by_group() ) :
											foreach ( $fields as $field ) :
												?><tr>
													<th scope="row" class="check-column">
														<input type="checkbox" name="delete_cf[]" value="<?php echo esc_attr( $field->cf_id ) ?>" id="cb-select-<?php echo esc_attr( $field->cf_id ) ?>">
													</th>
													<td>
													...
													<td>
														...
														switch( $field->cf_type ){
															...
															case 11: esc_html_e( 'Text', 'adifier' ); break;
														}
														?>
													</td>
													...
													<td>
														...
														<input type="hidden" value="<?php echo esc_attr( $field->cf_id ) ?>" class="cf_order">
													</td>
												</tr><?php
											endforeach;
										else :
						...
						<div class="form-wrap">
							...
							<form action="<?php echo esc_url( add_query_arg( array() ) ); ?>" method="post">
								...
								<div class="form-field">
									...
									<p class="description"><?php esc_html_e( 'If type is Nested Values make sure that name of the field is in form LEVEL1|LEVEL2.. ( for example Make|Model - divider is pipe not backslash or forwardslash ). If type of field is Single Value Numeric you can add prefix or sufix like this [prefix]  (sufix) for example Mileage (km)', 'adifier' ); ?></p>
								</div>
								...
								<div class="form-field">
									...
									<select name="cf_type" id="cf_type">
										...
										<option value="11"><?php esc_html_e( 'Text', 'adifier' ); ?></option>
									</select>
									<p class="description"><?php esc_html_e( 'Select type of the field', 'adifier' ); ?></p>
								</div>								
	...
	*/
					...
					<table class="form-table">
						<tbody>
							...
							<tr class="form-field form-required">
								...
								<td>
									<input name="cf_description" id="cf_description" type="text" value="<?php echo stripslashes( esc_attr( $field->cf_description ) ); ?>" />
									<p class="description"><?php esc_html_e( 'Small description for the field ( max 255 chars ).', 'adifier' ); ?></p>
								</td>
							</tr>
							<tr class="form-field form-required">
								...
								<td>
									<select name="cf_type" id="cf_type">
										...
										<option value="11" <?php selected( $field->cf_type, '11' ); ?>><?php esc_html_e( 'Text', 'adifier' ); ?></option>
									</select>
									<p class="description"><?php esc_html_e( 'Select type of the field.', 'adifier' ); ?></p>
								</td>
							</tr>
	...
	private static function get_posted_custom_field() {
		...
		return array(
			...
			'cf_slug'    		=> $slug,
			'cf_orderby' 		=> sanitize_text_field( $_POST['cf_orderby'] ),
			'cf_description' 	=> sanitize_text_field( $_POST['cf_description'] ),
			'cf_type'  			=> sanitize_text_field( $_POST['cf_type'] ),
			'cf_fixed'  		=> sanitize_text_field( $_POST['cf_fixed'] ),
			'cf_order'			=> sanitize_text_field( $_POST['cf_order'] ),
			'cf_is_hidden'		=> sanitize_text_field( $_POST['cf_is_hidden'] ),
			'cf_is_mandatory'	=> sanitize_text_field( $_POST['cf_is_mandatory'] ),
		);
	}
	...
	private static function process_delete_custom_field() {
		...
		$cf_ids = !is_array( $_REQUEST['delete_cf'] ) ? (array)absint( $_GET['delete_cf'] ) : $_REQUEST['delete_cf'];
	...
	*/
	public static function save_cf_order(){
		...
		global $wpdb;
		$list = !empty( $_POST['list'] ) ? map_deep( $_POST['list'], 'sanitize_text_field' ) : array();
		$counter = 0;
		if( !empty( $list ) ){
