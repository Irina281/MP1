...
class Adifier_Custom_Fields_Advert{
	...
	public static function launch(){
		...
		add_action( 'wp_ajax_adifier_get_cf', 'Adifier_Custom_Fields_Advert::get_cf_ajax' );
	...
	*/
	...
	private static function is_mandatory( $field ){
		return $field->cf_is_mandatory == 1 ? ' *' : '';
	}
	...
	*/
	...
	private static function is_hidden( $field ){
		return $field->cf_is_hidden == 1 ? esc_html__( ' Field is not used in search', 'adifier' ): '';
	}	
		...
		echo '<div class="cf-field type_'.esc_attr( $field->cf_type ).'">';
			...
			if( $field->cf_type == 1 ){
				...
				<div class="cf-subfield">
					<div class="flex-wrap">
						<label for="<?php echo esc_attr( $field->cf_slug ) ?>"><?php echo esc_html( $field->cf_label ).self::is_mandatory( $field ) ?></label>
					...
					<div class="cf-values-wrap">
						...
						<p class="description"><?php echo stripslashes( esc_html( $field->cf_description ) ).self::is_hidden( $field ) ?></p>
					</div>
				</div>
			...
			//if field is type of single date
				...
				?>
				<label for="<?php echo esc_attr( $field->cf_slug ) ?>"><?php echo esc_html( $field->cf_label ).self::is_mandatory( $field ) ?></label>
				<input type="text" class="cf-datepicker" value="<?php echo !empty( $value ) ? esc_attr( date_i18n( get_option( 'date_format' ), $value ) ) : '' ?>"/>
				<input type="hidden" class="cf-datepicker-val" name="cf_fields[type_3][<?php echo esc_attr( $field->cf_slug ) ?>][value]" id="<?php echo esc_attr( $field->cf_slug ) ?>" value="<?php echo !empty( $value ) ? esc_attr( $value ) : '' ?>"/>
				<p class="description"><?php echo stripslashes( esc_html( $field->cf_description ) ).self::is_hidden( $field ) ?></p>
				<?php
			}
			...
			//if field is type of checkboxes 
				...
				<div class="cf-subfield">
					<div class="flex-wrap">
						<label class="label-bottom-margin" for="<?php echo esc_attr( $field->cf_slug ) ?>"><?php echo esc_html( $field->cf_label ).self::is_mandatory( $field ) ?></label>
						<?php if( $field->cf_fixed == 0 || current_user_can('edit_published_posts') ): ?>
							<div class="styled-checkbox cf-no-terms">
								<input type="checkbox" class="no-terms-check" id="<?php echo esc_attr( $field->cf_slug ) ?>_no_terms" />
								<label for="<?php echo esc_attr( $field->cf_slug ) ?>_no_terms"><?php esc_html_e( 'Value not found?', 'adifier' ) ?></label>
			...
			//regular text
				...
				?>
				<label for="<?php echo esc_attr( $field->cf_slug ) ?>"><?php echo esc_html( $field->cf_label ).self::is_mandatory( $field ) ?></label>
				<input type="text" name="cf_fields[type_11][<?php echo esc_attr( $field->cf_slug ) ?>][value]" id="<?php echo esc_attr( $field->cf_slug ) ?>" value="<?php echo esc_attr( $value ) ?>"/>
				<p class="description"><?php echo stripslashes( esc_html( $field->cf_description ) ).self::is_hidden( $field ) ?></p>
				<?php
			}					
		echo '</div>';
	...
	*/
	public static function nested_select( $field, $selected_term_ids, $parent = 0, $depth = 0 ){
		...
		<div class="nested-field-wrap <?php echo  $depth > 0 ? esc_attr( 'cf-nested depth_'.$depth) : esc_attr( '' ) ?>">
			<div class="flex-wrap">
				<label for="<?php echo esc_attr( $field->cf_slug ) ?>-<?php echo esc_attr( $depth ) ?>"><?php echo esc_html( $label[$depth] ).self::is_mandatory( $field ) ?></label>
				<?php if( $field->cf_fixed == 0 || current_user_can('edit_published_posts') ): ?>
					<div class="styled-checkbox cf-no-terms">
	...
	*/
	public static function get_subfield(){
		...
		$depth = !empty( $_POST['depth'] ) ? sanitize_text_field( $_POST['depth'] ) : '';
	...
	*/
	...
	public static function get_cf_ajax(){
		if( !isset( $_REQUEST['adifier_nonce'] ) || !wp_verify_nonce( $_REQUEST['adifier_nonce'], 'adifier_nonce' ) ){
			die();
		}
	...
	*/
	public static function get_cf( $terms = array(), $post_id = 0 ){
		global $wpdb;
		$terms = !empty( $_POST['terms'] ) ? map_deep( $_POST['terms'], 'sanitize_text_field' ) : $terms;
		$post_id = !empty( $_POST['post_id'] ) ? sanitize_text_field( $_POST['post_id'] ) : $post_id;
		if( !empty( $terms ) ){
					...
					foreach( $fields as $slug => $data ){
						...
						if( $type !== 'type_5' ){
							...
							if( !empty( $data['new'] ) ){
								...
								foreach( $data['new'] as $term_name ){
									if( !empty( $term_name ) ){
										...
										$term = self::insert_term( sanitize_text_field( $term_name ), $slug );
										if( !is_wp_error( $term ) ){
							...
							//if it is single value for date or numeric value is treated just like data[new]
								...
								$term = self::insert_term( sanitize_text_field( $data['value'] ), $slug );
								if( !is_wp_error( $term ) ){
									$data['value'] = array( $term['term_id'] );
								}
							}
							else if( $type == 'type_4' || $type == 'type_8' || $type == 'type_11' ){
								$term = self::insert_term( sanitize_text_field( $data['value'] ) , $slug );
								if( !is_wp_error( $term ) ){
									$data['value'] = array( $term['term_id'] );
								}
							...
							for( $i=0; $i<$data['levels']; $i++ ){
								if( !empty( $data['value']['depth_'.$i] ) ){
									...
									$term = self::insert_term(
										sanitize_text_field( $data['new']['depth_'.$i] ),
										$slug,
										array(
