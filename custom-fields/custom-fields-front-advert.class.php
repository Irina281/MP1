...
class Adifier_Custom_Front_Advert{
	...
	*/
	private function _generate_array_fields( $field, $selected_terms ){
		...
		$list = array();
		if( in_array( $field->cf_type, array(1, 2, 9, 10, 11) ) ){
			$data['label'] = $field->cf_label;
			foreach( $selected_terms as $term ){
				$list[] = $term->name;
			}
			$data['value'] = join( ', ', $list );
			if( $field->cf_type == 11 && stristr($data['value'], 'http')){
				$data['value'] = '<a href="'.esc_url( $data['value'] ).'" rel="nofollow" target="_blank">'.$data['value'].'</a>';
			}
		}
		...
		else if( $field->cf_type == 4 || $field->cf_type == 8 ){
