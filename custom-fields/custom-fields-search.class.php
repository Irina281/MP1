		...
		if( $field->cf_type == 1 ){
			...
			<div class="filter-group cf-field type_<?php echo esc_attr( $field->cf_type ); ?> cf_<?php echo esc_attr( $field->cf_slug ) ?>">
				...
				<div class="date-range">
					<input type="text" value="<?php echo !empty( $min_date ) ? esc_attr( date_i18n( get_option( 'date_format' ), $min_date ) ) : '' ?>" class="cf-datepicker" placeholder="<?php esc_attr_e( 'From', 'adifier' ) ?>"/>
					<input type="hidden" name="cf_fields[<?php echo esc_attr( $field->cf_slug ) ?>][min]" value="<?php echo esc_attr( $min_date ) ?>" />
					<input type="text" value="<?php echo !empty( $max_date ) ? esc_attr( date_i18n( get_option( 'date_format' ), $max_date ) ) : '' ?>" class="cf-datepicker" placeholder="<?php esc_attr_e( 'To', 'adifier' ) ?>"/>
					<input type="hidden" name="cf_fields[<?php echo esc_attr( $field->cf_slug ) ?>][max]" value="<?php echo esc_attr( $max_date ) ?>" />
				</div>
			</div>
			<?php
		}
