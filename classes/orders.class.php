...
class Adifier_Order{
	...
	*/
	...
	static public function adifier_number_manuel_order() {
	    global $wpdb;
	    $count = $wpdb->get_var("SELECT COUNT(meta_id) FROM {$wpdb->posts} LEFT JOIN {$wpdb->postmeta} ON ID = post_id WHERE post_status = 'publish' AND meta_key = 'order_paid' AND meta_value = 'no'");
	    ...
	    switch ( $column ) {
	        ...
	        case 'paid' :
	        	...
	        	switch( $paid ){
					...
					case 'canceled'		: esc_html_e( 'Canceled', 'adifier' ); break;
	        	}
	        	break;
	    }
	}
	...
	*/
	...
	static public function get_buyer_information( $order_id, $user_id )
	{
		$order_details = self::get_order_details( $order_id );
		if( empty( $order_details['buyer_location'] ) )
		{
			$order_details['buyer_name'] = adifier_author_name( $user_id );
			$order_details['buyer_location'] = self::_get_buyer_location( $user_id );
			update_post_meta( $order_id, '_order_json_details', $order_details );
		}

		$info[] = $order_details['buyer_name'];
		$info = array_merge( $info, $order_details['buyer_location'] ?: [] );
		
		return array_filter( $info );
	}
	
	static public function display_buyer_information( $buyer_information ){
		echo '<div>' . implode('</div><div>', $buyer_information ) . '</div>';
	}
	...
	*/
	static public function buyer_box_callback( $post ){
		$buyer_information = self::get_buyer_information( $post->ID, $post->post_author );
		self::display_buyer_information( $buyer_information );
	}
	...
	*/
	static public function order_save( $post_id, $post, $update ){
		...
		if( !isset( $_POST['adifier_refund_order'] ) ){
		    ...
		    if( ( empty( $old_paid ) || $old_paid == 'no' || $old_paid == 'canceled' ) && $order_paid == 'yes' ){
	...
	*/
	static private function _apply_subscription( $user_id, $time ){
		...
		$subscribe = ( $subscribe == 0 || $subscribe < current_time( 'timestamp' ) ) ? current_time( 'timestamp' ) : $subscribe;
	...
	*/
	static public function mail_order( $order, $order_id ){
		...
		ob_start();
		$invoice_data = adifier_get_option( 'invoice_data' );
		$buyer_information = self::get_buyer_information( $order_id, $user_id );
	...
	*/
	static public function update_get_option_num_start(){
		$invoice_num_start = adifier_get_option( 'invoice_num_start' );
		if( empty( $invoice_num_start ) && class_exists( 'Redux' ) ){
			Redux::set_option('adifier_options','strict_sequential_numbering', 'no');
	...
	*/
	static public function update_redux_invoice_num_start(){
		$invoice_num_start = get_option( 'invoice_num_start' );
		if( !empty( $invoice_num_start ) && class_exists( 'Redux' ) ){
			Redux::set_option('adifier_options','invoice_num_start', $invoice_num_start);
		}
	}
	...
	*/
	...
	static private function _get_buyer_location( $user_id )
	{
		$location = [];

		if( adifier_get_option( 'use_google_location' ) == 'yes' ){
			$location = adifier_get_geo_location( $user_id );
		}
		else if( adifier_get_option( 'use_predefined_locations' ) == 'yes' ){
			$location = [];
			$terms = adifier_get_predef_location( $user_id );
			if( !empty( $terms ) )
			{
	...
	*/
	static public function create_order( $args ){
		...
		if( !empty( $post_id ) ){
			...
			$order['buyer_name'] = adifier_author_name( $order['userId'] );
			$order['buyer_location'] = self::_get_buyer_location( $order['userId'] );
	...
	*/
	static public function get_order(){
		...
		$order = map_deep( $order, 'sanitize_text_field' );
