...
class Adifier_Advert_Query extends WP_Query {
	...
	*/
	...
	function posts_where( $sql ) {
		...
		/* Filtering by sale */
		if( $this->args['sale'] ){
			$sql .= " AND type = '1' AND sale_price > 0 ";
		}
...
function adifier_pre_get_posts_author( $query ){
	...
	if( $query->is_author() && !is_single() && ( empty( $_GET['screen'] ) || !is_user_logged_in() ) && function_exists( 'adifier_create_post_types' )  ){
		...
		if( !empty( $_GET['filter-seller-ads'] ) ){
			$query->set( 's', sanitize_text_field( $_GET['filter-seller-ads'] ) );
		}
		$query->set( 'order', 'DESC' );
		$query->query_vars['posts_per_page'] = adifier_get_option( 'adverts_per_page_author' );
	}
	else if( $query->is_main_query() && ( is_tax( 'advert-category' ) || is_tax( 'advert-location' ) ) ){
...
function adifier_advert_category_posts_request( $sql, $query ){
	if( $query->is_main_query() && ( is_tax( 'advert-category' ) || is_tax( 'advert-location' ) ) ){
		global $wpdb;
...
add_filter( 'posts_request', 'adifier_advert_category_posts_request', 10 , 2);
}
...
function adifier_advert_category_tax_fields( $sql, $query ){
	if( $query->is_main_query() && ( is_tax( 'advert-category' ) || is_tax( 'advert-location' ) || ( is_admin() && !empty( $_GET['adifier_admin_filter'] ) ) ) ){
		$sql = Adifier_Advert_Query::posts_main_fields( $sql );
	}

	return $sql;
}
add_filter( 'posts_fields', 'adifier_advert_category_tax_fields', 10 , 2 );
}
...
function adifier_advert_category_tax_join( $sql, $query ) {
	...
	if( $query->is_main_query() && ( $author_frontend_browse || is_tax( 'advert-category' ) || is_tax( 'advert-location' ) || ( is_admin() && !empty( $_GET['adifier_admin_filter'] ) ) ) ){
		$sql = Adifier_Advert_Query::posts_main_join( $sql );
	}

	return $sql;
}
add_filter( 'posts_join', 'adifier_advert_category_tax_join', 10, 2 );
}
...
function adifier_advert_category_tax_where( $sql, $query ){
	global $wpdb;
	if( $query->is_main_query() && ( is_tax( 'advert-category' ) || is_tax( 'advert-location' ) ) ){
	...
	else if(  $query->is_main_query() && is_author() && function_exists( 'adifier_create_post_types' ) ){
...
add_filter( 'posts_where', 'adifier_advert_category_tax_where', 10, 2 );
}
...
function adifier_get_advert_meta( $post_id, $meta_key, $single = false ){
	...
	if( $meta_key == 'location' ){
		...
		if( !$single ){
			return !empty( $data[$meta_key] ) ? array( $data[$meta_key] ) : array();
		}
		else{
			return !empty( $data[$meta_key] ) ? $data[$meta_key] : '';
		}
	}
}
}
...
function adifier_save_advert_meta( $post_id, $meta_key, $meta_value ){
	...
	if( $meta_key == 'location' ){
		...
		if( empty( $meta_value['lat'] ) && !empty( $_POST['post_author'] ) ){
			$meta_value = get_user_meta( sanitize_text_field( $_POST['post_author'] ), 'location', true );
		}
		else{
...
function adifier_get_advert_price( $post_id = 0 ){
	...
	$price_html = '<div class="price">';
		...
		if( $type == '1' ){
			...
			$price_html .= '<span class="text-price">'.esc_html__( 'Partner', 'adifier' ).'</span>';
		}
		else if( $type == '6' ){
			$price = adifier_get_advert_meta( $post_id, 'price', true );
...
* Get advert phone
*/
if( !function_exists('adifier_get_display_phone') ){
function adifier_get_display_phone(){	
	$enable_phone_verification = adifier_get_option( 'enable_phone_verification' );
	if( is_singular( 'advert' ) ){
		$phone = get_post_meta( get_the_ID(), 'advert_phone', true );
		$author_id = empty( $post->author ) ? get_the_author_meta('ID') : $post->author;
	}
	else{
	...
	$hide_phone = get_user_meta( $author_id, 'hide_phone', true );
	if( !empty( $hide_phone ) && empty( $phone ) ){
		return '';
	}	
	...
	if( $enable_phone_verification == 'yes' && get_user_meta( $author_id, 'af_phone_verified', true ) != '1' ){	
		$phone = '';
	}
...
/**
 * Update page number for browse ads
 */
if( !function_exists( 'adifier_page_title' ) ){
function adifier_page_title( $parts ){
    if( !empty( $_GET['af_page'] ) && $_GET['af_page'] > 1 ){
        $parts = array_slice( $parts, 0, 1, true) + [ 'page' => sprintf( esc_html__( 'Page %s', 'adifier' ), $_GET['af_page'] ) ] + array_slice( $parts, 1, count( $parts ) - 1, true);
    }

    return $parts;
}
add_filter( 'document_title_parts', 'adifier_page_title', 10, 2 );
}
?>