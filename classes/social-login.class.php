...
class Adifier_Social_Login{
	...
	public function callback(){
		...
		else if( !empty( $_GET['adifier-facebook-delete'] ) )
		{
			$this->facebook_delete();
		}
		else if( !empty( $_GET['adifier-facebook-delete-status'] ) )
		{
			$this->facebook_delete_status();
		}
					...
					if( !empty( $data['data']['id'] ) && !empty( $data['data']['secret'] ) ){
						...
						<li>
							<a href="javascript:void(0);" class="<?php echo esc_attr( $network ) ?>">
								<img src="<?php echo get_theme_file_uri( 'images/'.$network.'.png' ) ?>" width="<?php echo esc_attr( $data['width'] ) ?>" height="<?php echo esc_attr( $data['height'] ) ?>">
	...
	private function parse_signed_request( $signed_request )
	{
		...
		$secret = $this->networks['facebook']['data']['secret'];
	...
	private function facebook_delete()
	{
		$signed_request = sanitize_text_field( $_REQUEST['signed_request'] );
		$data = $this->parse_signed_request( $signed_request );
		if( !empty( $data ) )
		{
			$user_id = $this->_get_registered_user( $data['user_id'] );
			if( !empty( $user_id ) )
			{
		...
		echo json_encode([
			'url' 				=> home_url( '/index.php?adifier-facebook-delete-status=1' ),
			'confirmation_code' => $confirmation_code ?? '404'
		]);

		die();
	}

	private function facebook_delete_status()
	{
		if( !empty( $_REQUEST['confirmation_code'] ) && $_REQUEST['confirmation_code'] == '200' )
		{
			esc_html_e( 'All data associated with your account have been deleted', 'adifier' );
		}
