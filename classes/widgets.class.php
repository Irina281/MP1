...
class Adifier_Widget_Adverts extends Adifier_Widget{
	function __construct(){
		$this->properties =	array(
			...
			'fields' 		=> array(
				...
				array(
					'id' 			=> 'topads',
					'title' 		=> esc_html__( 'Top Ads Only', 'adifier' ),
                    'description'	=> esc_html__( 'Search among top ads only ( Ad Ids will have no effect )', 'adifier' ),
					'type' 			=> 'select',
					'options'		=> array(
	...
	function widget($args, $instance) {
		...
		if( !empty( $advert_ids ) ){
            ...
            if( !empty( $topads ) && $topads == 'yes' ){
                $query['post__in'] = adifier_topads_ids_list();
            }
	...
	function __construct(){
		$this->properties =	array(
			...
			'fields' 		=> array(
				...
				array(
					'id' 			=> 'count',
					'title' 		=> esc_html__( 'Show Count', 'adifier' ),
					'type' 			=> 'select',
					'options'		=> array(
						'yes'			=> esc_html__( 'Yes', 'adifier' ),
						'no'			=> esc_html__( 'No', 'adifier' ),
					),
					'description'	=> esc_html__( 'Enable or disable display of count of ads', 'adifier' ),
				),
			)
		);
		parent::__construct();
					...
					<li class="flex-wrap">
						...
						<?php if( $count == 'yes' ){ ?>
							<span>
								...
								echo  $term_count.' '._n( 'ad', 'ads', $term_count, 'adifier' ); 
								?>
							</span>
						<?php } ?>
					</li>
...
if( !class_exists('Adifier_Widget_Share') ){
class Adifier_Widget_Share extends Adifier_Widget{
	function __construct(){
		$this->properties =	array(
			'id' 			=> 'adifier_share',
			'title' 		=> esc_html__('Adifier Share Posts/Pages','adifier'),
			'description' 	=> esc_html__('Display share on posts or pages','adifier'),
			'fields' 		=> array(
				array(
					'id' 		=> 'title',
					'title' 	=> esc_html__( 'Title', 'adifier' ),
...
function adifier_widgets_register() {
	...
	register_widget('Adifier_Widget_Share');
}
