...
class Adifier_MakeCommerce{
	...
	*/
	static public function start_payment(){
		if( self::is_enabled() ){
            ...
            if( !empty( $_GET['adifier_verify_payment'] ) && $_GET['adifier_verify_payment'] == 'makecommerce' ){
                self::verify_payment();
            }
	...
	*/
	static public function render(){
		...
		?>	
		<li>
			<a href="javascript:void(0);" id="makecommerce-button" data-returnmessage="<?php esc_attr_e( 'You payment is being processed and once it is approved you will receive purchased goods', 'adifier' ) ?>">
	...
	*/
			...
			$transaction = self::http( 'transactions', array(
				'transaction' => array(
					...
					'transaction_url' 	=> array(
						'return_url' => array(
						  'method' 	=> 'POST',
						  'url'		=> add_query_arg( array( 'adifier_verify_payment' => 'makecommerce' ), $_POST['redirectUrl'] ).'#makecommerce-return'
						)
					)
				),
			    'customer' => array(
    ...
    /*
    Verify payment of makecommerce
    */
    static public function verify_payment(){
		$mac = !empty( $_POST['mac'] ) ? $_POST['mac'] : false;
		if( $mac ){
			$mc_api_secret = adifier_get_option( 'mc_api_secret' );
			$test_mac = strtoupper(hash('sha512', stripcslashes( $_POST['json'] ).$mc_api_secret));
			if( $_POST['mac'] == $test_mac ){
				self::execute_payment( false );
			}
		}
    }		
	...
	*/
	static public function execute_payment( $echo_result = true ){
		$data = $echo_result == false ? json_decode( stripslashes( $_POST['json'] ), true ) : $_POST['data'];
		if( !empty( $data ) ){
