...
function adifier_admin_conversations(){
	if( !empty( $_POST ) && !wp_verify_nonce( $_POST['adifier_nonce'], 'adifier_nonce' ) ){
		die();
	}

	global $wpdb;
	$con_id = !empty( $_GET['con_id'] ) ? sanitize_text_field( $_GET['con_id'] ) : '';

	/* if array with con_ids is submited then it means that we need to delte conversations and their messages */
	$con_ids = !empty( $_POST['con_ids'] ) ? map_deep( $_POST['con_ids'], 'sanitize_text_field' ) : array();
	if( !empty( $con_ids ) ){
	...
	$message_ids = !empty( $_POST['message_ids'] ) ? map_deep( $_POST['message_ids'], 'sanitize_text_field' ) : array();
