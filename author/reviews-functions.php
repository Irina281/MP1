...
function adifier_fetch_ajax_reviews(){
	if( !isset( $_REQUEST['adifier_nonce'] ) || !wp_verify_nonce( $_REQUEST['adifier_nonce'], 'adifier_nonce' ) ){
		...
		$reviews = new Adifier_Reviews_Frontend(array(
			...
			'paged'		=> !empty( $_POST['page'] ) 		? sanitize_text_field( $_POST['page'] ) 		: '',
			'author_id'	=> !empty( $_POST['author_id'] ) 	? sanitize_text_field( $_POST['author_id'] ) 	: ''
		));
...
function adifier_write_review(){
	...
	if( !adifier_bad_words_filter( $review ) )
	{
		...
		$response['message'] = '<div class="alert-error">'.esc_html__( 'Form contains bad words.', 'adifier' ).'</div>';
	}
...
function adifier_review_response(){
	...
	$review_response = sanitize_textarea_field( $_POST['review_response'] );
