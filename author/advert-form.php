<div class="author-panel">
	...
	if( Adifier_Twilio::is_phone_verification_needed() ){
	...
	else if( !adifier_mandatory_profile_fields_populated() ){
		?>
		<div class="white-block white-block-extra-padding">
			<div class="white-block-content">
				<div class="text-center author-no-listing">
					<h5>
						<?php  esc_html_e( 'Populate mandatory fields in your profile prior to posting', 'adifier' ); ?>
					</h5>
					<br />
					<a class="af-button" href="<?php echo esc_url( add_query_arg( 'screen', 'profile', $author_url ) ) ?>">
						<?php esc_html_e( 'Go to profile', 'adifier' ) ?>
					</a>					
				</div>
			</div>
		</div>
		...
		$user_type = get_user_meta( 'user_type');
		...
		if( !empty( $id ) ){
			...
			if( !empty( $advert ) ){
				...
				$advert_data['user_type'] 			= get_user_meta( $id, 'user_type', true );
							...
							<div class="white-block white-block-extra-padding">
								...
								<div class="white-block-content">
									...
									<?php if( !empty( $data['next_page'] ) ): ?>
										<a href="javascript:void(0);" class="bidding-history af-button" data-advertid="<?php echo esc_attr( $id ) ?>" data-bidpage="2" data-full="<?php echo adifier_is_expired( $id ) ? true : false ?>">
					...
					<div class="white-block white-block-extra-padding">
						...
						<div class="white-block-content">
							<div class="form-group">
								<label for="title"><?php esc_html_e( 'Name *', 'adifier' ) ?></label>
								<input type="text" id="title" name="title" value="<?php echo esc_attr( $title ) ?>" placeholder="<?php esc_html_e( 'Precise title is always better', 'adifier' ); ?>" maxlength="<?php echo esc_attr( adifier_get_option( 'max_title_length' ) ) ?>" />
							</div>
										...
										<div class="styled-select">
											<select name="type" id="type">
												...
												<?php if(  $user_type== 'Partner' ): ?>
                                                 <option value="5" ><?php echo sprintf( esc_html__( 'Partner', 'adifier' ) )?></option>
                                                 <?php endif; ?>
							...
							<?php if( adifier_is_allowed_ad_type(6) && ( $screen == 'new' || $type == '6' )  ): ?>
								<div class="row advert-type-6 <?php echo ( adifier_is_allowed_ad_type(1) || adifier_is_allowed_ad_type(2) || adifier_is_allowed_ad_type(3) || adifier_is_allowed_ad_type(4) || adifier_is_allowed_ad_type(5) ) && $type !== '6' ? esc_attr( 'hidden' ) : esc_attr( '' ) ?>">
									<div class="col-sm-<?php echo count( $currencies ) > 1 ? esc_attr( '4' ) : esc_attr( '6' ) ?>">
										<div class="form-group">											
											<div class="flex-wrap">
							...
							<?php if( adifier_is_allowed_ad_type(7) && ( $screen == 'new' || $type == '7' )  ): ?>
								<div class="row advert-type-7 <?php echo ( adifier_is_allowed_ad_type(1) || adifier_is_allowed_ad_type(2) || adifier_is_allowed_ad_type(3) || adifier_is_allowed_ad_type(4) || adifier_is_allowed_ad_type(5) || adifier_is_allowed_ad_type(6) ) && $type !== '7' ? esc_attr( 'hidden' ) : esc_attr( '' ) ?>">
									<div class="col-sm-<?php echo count( $currencies ) > 1 ? esc_attr( '4' ) : esc_attr( '6' ) ?>">
										<div class="form-group">
											<div class="flex-wrap">
							...
							<?php if( adifier_is_allowed_ad_type(1) && ( $screen == 'new' || $type == '1' )  ): ?>
								<div class="column advert-type-1" >
							...
							<div class="form-group">
								...
								<div class="images-uploader">
									...
									<?php $max_image_size = adifier_get_image_max_upload_size(); ?>
									<p class="description">
										<?php echo sprintf( esc_html__( 'Prepare images before uploading. Upload images larger than %spx x %spx.', 'adifier'), adifier_get_option( 'single-slider_width' ), adifier_get_option( 'single-slider_height' ) ); ?>
										<?php
										if( !empty( $max_images ) ){
							...
							<div class="form-group no-margin">
								...
								add_filter( 'tiny_mce_before_init', 'adifier_frontend_editor_style');
								wp_editor( $description, 'description', array( 
									'media_buttons' 	=> false, 
									'quicktags' 		=> false 
								) );
								remove_filter( 'mce_buttons', 'adifier_remove_buttons_from_tinymce');
								remove_filter( 'tiny_mce_before_init', 'adifier_frontend_editor_style');
								?>
							</div>
    ...
    <div class="white-block white-block-extra-padding">
        ...
        <div class="white-block-content">
            ...
            if( $category_selection == 'combined' ){
                    ...
                    <div class="form-group no-margin">
                        ...
                        <?php if ( $type == '1' )  { ?>
                            <select name="category" id="category" class="select2-enabled">
                                ...
                                <?php
                                if( adifier_is_allowed_ad_type(1) && ( $screen == 'new' || $type == '1') ) {
                                    addifier_hierarchy_select_taxonomy( $categories, 0, $category_ids );
                                } else if (adifier_is_allowed_ad_type(5) && ( $screen == 'new' || $type == '5') ) { 
									?>
									<select name="category" id="category" class="select2-enabled">
										<option value="Partner"><?php esc_html_e( 'Partner', 'adifier' ) ?></option>
									</select>
                                <?php } ?>
                            </select>
                        <?php } ?>
                ...
                adifier_taxonomy_multifilter( esc_html__( 'Category *', 'adifier' ), 'category', 'advert-category', $category_ids, false );
            }
            ?>
                ...
                ?>
					...
					<div class="white-block white-block-extra-padding">
						...
						<div class="white-block-content">
										...
										if( $use_predefined_locations == 'yes' ){
											...
											<div class="form-group <?php echo ( $use_google_location == 'no' && !empty( $id ) ) ? esc_attr( '' ) : esc_attr( 'margin-top-15' ) ?>">
												...
												if( $location_selection == 'combined' ){
													...
													adifier_taxonomy_multifilter( esc_html__( 'Location *', 'adifier' ), 'location_id', 'advert-location', $location_ids, false );
												}
												?>
											</div
											><?php
							...
							<?php
							if( $enable_phone_verification !== 'yes' && $phone_visibility !== '3' ):
								?>
								<div class="form-group no-margin">
						...
						<div class="white-block white-block-extra-padding">
							<div class="white-block-content">
								...
								<div class="ajax-form-images hidden">
									<div class="alert-info"><?php esc_html_e( 'Uploading images...', 'adifier' ) ?><span class="upload-progress">0%</span></div>
								</div>
