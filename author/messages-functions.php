...
class Adifier_Conversation_Query{
	...
	public function __construct(){
		...
		$this->args = array(
			'per_page'		=> 50,
			'paged'			=> !empty( $_GET['page'] ) ? sanitize_text_field( $_GET['page'] ) : 1,
...
class Adifier_Conversations{
	...
	*/
	static public function initiate_conversation(){
		...
		$con_id = !empty( $_POST['con_id'] ) ? sanitize_text_field( $_POST['con_id'] ) : ''; /* This is used if contact modal is opened and users sends multiple messages */
		if( !empty( $advert_id ) && !empty( $message ) && !adifier_bad_words_filter( $message ) ){
			if( !empty( $con_id ) ){
				$response = self::_initiate_sent_message( $con_id, $message );
			}
			else{
				$response = self::initiate_conversation_write_message(array(
					...
					'recipient_id'	=> !empty( $_POST['buyer_id'] ) ? sanitize_text_field( $_POST['buyer_id'] ) : get_post_field( 'post_author', $advert_id )
				), $message);
			}
		}
		else{
...
class Adifier_Messages{
	...
	*/
	static public function send_message(){
		...
		if( !adifier_bad_words_filter( $message ) )
		{
			...
			if( !empty( $conversation ) && in_array( get_current_user_id(), array( $conversation->sender_id, $conversation->recipient_id ) ) ){
				$message_id = self::save_message( $con_id, $message );
				...
				if( ( $message_id ) ){
					...
					self::send_unread_mail( $conversation, $message );
