...
$filter 	= !empty( $_GET['filter'] ) ? sanitize_text_field( $_GET['filter'] ) : '';
...
$enable_purchase = adifier_mandatory_profile_fields_populated();
?>
<div class="author-panel">
	<?php
		if( !$enable_purchase && $enable_promotions == 'yes' )
		{
			?>
			<div class="alert-error">
				<?php 
					echo sprintf( 
						esc_html__( 'Go to your %s and populate mandatory fields in order to purchase promotions', 'adifier' ), 
	...
	<div class="white-block white-block-extra-padding">
		...
		<div class="white-block-content">
						...
						<div class="styled-select inline-select change-submit">
							<select name="type">
								...
								<?php if( adifier_is_allowed_ad_type(5) ): ?>
									<option value="5" <?php selected( $type, '5' ) ?>><?php esc_html_e( 'Partner', 'adifier' ) ?></option>
								<?php endif; ?>
			...
			<?php
			$cur_page = !empty( $_GET['cpg'] ) ? sanitize_text_field( $_GET['cpg'] ) : 1;
			$run_query = true;
							...
							<div class="profile-advert advert-<?php echo esc_attr( get_the_ID() ); ?>" <?php echo adifier_get_advert_attr_data(); ?>>
								...
								<div class="action <?php echo $screen !== 'ads' ? esc_attr( 'action-expand' ) : '' ?>">
									...
									<a href="<?php the_permalink( $edit_id ) ?>" target="_blank" title="<?php esc_attr_e( 'View Ad', 'adifier' ) ?>">
										...
										<span class="icon-caption animation"><?php  esc_html_e( 'View', 'adifier' )?></span>
									</a>
									<?php if( $screen == 'ads' ): ?>
										<a href="<?php echo esc_url( add_query_arg( array( 'screen' => 'edit', 'id' => $edit_id ), $author_url ) ) ?>" title="<?php esc_attr_e( 'Edit Ad', 'adifier' ) ?>">
											...
											<span class="icon-caption animation"><?php esc_html_e( 'Edit', 'adifier' ) ?></span>
										</a>
										<a href="javascript:void(0);" class="profile-delete-advert" data-id="<?php echo esc_attr( get_the_ID() ) ?>" data-confirm="<?php esc_attr_e( 'Are you sure?', 'adifier' ) ?>" title="<?php esc_attr_e( 'Delete Ad', 'adifier' ) ?>">
											...
											<span class="icon-caption animation"><?php  esc_html_e( 'Delete', 'adifier' )?></span>
										</a>
										<?php if( !adifier_is_expired() && get_post_status() == 'publish' && $enable_promotions == 'yes' && $enable_purchase ): ?>
											<a href="javascript:void(0)" class="profile-promote-advert" data-id="<?php echo esc_attr( get_the_ID() ) ?>" title="<?php esc_attr_e( 'Promote Ad', 'adifier' ) ?>">
												...
												<span class="icon-caption animation"><?php  esc_html_e( 'Promote', 'adifier' )?></span>
											</a>
										<?php endif; ?>
									<?php elseif( $screen == 'favorites' ): ?>
										<a href="javascript:void(0)" class="remove-favorites" data-id="<?php echo esc_attr( get_the_ID() ) ?>" title="<?php esc_attr_e( 'Remove From Favorites', 'adifier' ) ?>" data-confirm="<?php esc_attr_e( 'Are you sure?', 'adifier' ) ?>">
											...
											<span class="icon-caption animation"><?php  esc_html_e( 'Unfavorite', 'adifier' )?></span>
										</a>										
									<?php endif; ?>
								</div>
							</div>
				...
				<?php
					...
					if( $screen == 'ads' ){
						...
						<div class="text-center author-no-listing">
							<?php if( empty( $filter ) && empty( $keyword ) ): ?>
								<a href="<?php echo esc_url( add_query_arg( 'screen', 'new', $author_url ) ) ?>">
									<i class="aficon-question-circle"></i>
								</a>
								<h6><?php esc_html_e( 'Offer your product/service by submitting your first advert', 'adifier' ) ?></h6>
							<?php else: ?>
								<i class="aficon-question-circle"></i>
						...
						<div class="text-center author-no-listing">
							...
							<h6><?php $screen == 'auctions' ? esc_html_e( 'Currently you do not participate in any auction.', 'adifier' ) : esc_html_e( 'Currently you do not have any favorite ads, add some.', 'adifier' ) ?></h6>
						</div>						
						<?php
					}
				}
