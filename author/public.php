...
$filter = !empty( $_GET['filter-seller-ads'] ) ? sanitize_text_field( $_GET['filter-seller-ads'] ) : '';
...
$address = adifier_show_profile_address( $author->ID );
			...
			<div class="col-sm-4">
				...
				if( adifier_get_option( 'use_google_location' ) == 'yes' && !empty( $location['lat'] ) && !empty( $location['long'] ) ){
					...
					<div class="white-block map-wrapper">
						<div class="location-map" data-zoom="<?php echo esc_attr( adifier_get_option( 'profile_location_zoom' ) ) ?>" data-lat="<?php echo esc_attr( $location['lat'] ) ?>" data-long="<?php echo esc_attr( $location['long'] ) ?>"></div>
